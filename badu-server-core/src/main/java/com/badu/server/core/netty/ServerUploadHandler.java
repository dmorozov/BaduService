...
public class ServerUploadHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, HttpObject msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            try {
                ...
                LOG.error("Unable to decode request", e1);
    ...
    private void writeResponse(Channel channel) {
        ...
        boolean close = HttpHeaderValues.CLOSE.equals(StringUtils.toLowerCase(request.headers().get(CONNECTION)))
                || request.protocolVersion().equals(HttpVersion.HTTP_1_0)
                && !HttpHeaderValues.KEEP_ALIVE.equals(StringUtils.toLowerCase(request.headers().get(CONNECTION)));
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        LOG.warn(responseContent.toString(), cause);
        ctx.channel().close();
    }
}
